...
export default function Orders() {
  ...
  const getPaymentStatus = (order) => {
    if (!['VNPAY', 'SHOPEEPAY'].includes(order.paymentMethod)) {
      return {
        label: order.paymentStatus === 'Paid' ? 'Đã TT' : 'Chưa TT',
        status: order.paymentStatus
      };
    }

    const payment = payments.find(
      p => p.orderID === order.orderID && p.paymentMethod === order.paymentMethod
    );

    if (!payment) {
      return { label: 'Chưa xác nhận', status: 'Pending' };
    }

    return payment.status === 'Success'
      ? { label: 'Đã TT', status: 'Paid' }
